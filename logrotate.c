...
int createOutputFile(char *fileName, int flags, struct stat *sb, acl_type acl, int force_mode) {
	...
	fd = open(fileName, (flags | O_EXCL | O_NOFOLLOW), (S_IRUSR | S_IWUSR) & sb->st_mode);
...
static int compressLogFile(char *name, struct logInfo *log, struct stat *sb)
{
	...
	fullCommand = alloca(sizeof(*fullCommand) * (log->compress_options_count + 2));
...
static int mailLog(struct logInfo *log, char *logFile, char *mailCommand, char *uncompressCommand, char *address, char *subject) {
	int mailInput;
...
static int mailLogWrapper(char *mailFilename, char *mailCommand, int logNum, struct logInfo *log) {
	/* if the log is compressed (and we're not mailing a
	* file whose compression has been delayed), we need
	* to uncompress it */
	if ((log->flags & LOG_FLAG_COMPRESS) && !(log->flags & LOG_FLAG_DELAYCOMPRESS)) {
		if (mailLog(log, mailFilename, mailCommand, log->uncompress_prog, log->logAddress, (log->flags & LOG_FLAG_MAILFIRST) ? log->files[logNum] : mailFilename)) {
			return 1;
		}
	}
	else {
		if (mailLog(log, mailFilename, mailCommand, NULL, log->logAddress, mailFilename)) {
			return 1;
		}
	}
	return 0;
...
static int sparse_copy(int src_fd, int dest_fd, struct stat *sb, const char *saveLog, const char *currLog) {
	...
	if (force) {
		...
		case ROT_WEEKLY:
			...
			state->doRotate = ((now.tm_wday < state->lastRotated.tm_wday)
				|| ((mktime(&now) - mktime(&state->lastRotated)) > (7 * 24 * 3600)));
			if (!state->doRotate) {
				message(MESS_DEBUG, "  log does not need rotating "
...
int prerotateSingleLog(struct logInfo *log, int logNum, struct logState *state, struct logNames *rotNames) {
	struct tm now = *localtime(&nowSecs);
...
#define PATTERN_LEN (DATEEXT_LEN * 2)
	...
	if (log->extension && strncmp(& (rotNames->baseName[strlen(rotNames->baseName) - strlen(log->extension)]),
		log->extension, strlen(log->extension)) == 0) {
		char *tempstr;

		fileext = log->extension;
		tempstr = calloc(strlen(rotNames->baseName) - strlen(log->extension) + 1, sizeof(char));
		strncat(tempstr, rotNames->baseName, strlen(rotNames->baseName) - strlen(log->extension));
	...
	if (log->flags & LOG_FLAG_COMPRESS && log->flags & LOG_FLAG_DELAYCOMPRESS) {
		if (log->flags & LOG_FLAG_DATEEXT) {
			...
			if (asprintf(&glob_pattern, "%s/%s%s%s", rotNames->dirName, rotNames->baseName, dext_pattern, fileext) < 0) {
			...
			if (!rc && globResult.gl_pathc > 0) {
				...
				if (asprintf(&oldName, "%s/%s.%d%s", rotNames->dirName, rotNames->baseName, logStart, fileext) < 0) {
	...
	rotNames->firstRotated = malloc(strlen(rotNames->dirName) + strlen(rotNames->baseName) +
		strlen(fileext) + strlen(compext) + DATEEXT_LEN + 2 );

	if (log->flags & LOG_FLAG_DATEEXT) {
		/* glob for compressed files with our pattern
		 * and compress ext */
		if (asprintf(&glob_pattern, "%s/%s%s%s%s", rotNames->dirName, rotNames->baseName, dext_pattern, fileext, compext) < 0) {
		...
		if (!rc) {
			...
			for (i = 0; i < globResult.gl_pathc; i++) {
				if (!stat((globResult.gl_pathv)[i], &fst_buf)) {
					if ((i <= ((int) globResult.gl_pathc - rotateCount)) || ((log->rotateAge > 0)
						&& (((nowSecs - fst_buf.st_mtime) / 60 / 60 / 24) > log->rotateAge))) {
						if (mail_out != -1) {
							char *mailFilename =
...
int rotateSingleLog(struct logInfo *log, int logNum, struct logState *state, struct logNames *rotNames) {
	int hasErrors = 0;
	struct stat sb;
	int fd;
#ifdef WITH_SELINUX
	...
	if (!hasErrors) {
		...
		if (!(log->flags & (LOG_FLAG_COPYTRUNCATE | LOG_FLAG_COPY))) {
			...
			if (log->flags & LOG_FLAG_TMPFILENAME) {
				...
				if (!debug && !hasErrors && rename(log->files[logNum], rotNames->finalName)) {
					message(MESS_ERROR, "failed to rename %s to %s: %s\n", log->files[logNum], tmpFilename,
						strerror(errno));
					hasErrors = 1;
				}
...
int postrotateSingleLog(struct logInfo *log, int logNum, struct logState *state, struct logNames *rotNames) {
	int hasErrors = 0;
	...
	if (!hasErrors && log->flags & LOG_FLAG_TMPFILENAME) {
		...
		hasErrors = copyTruncate(tmpFilename, rotNames->finalName, &state->sb, LOG_FLAG_COPY);
	...
	if (!hasErrors && (log->flags & LOG_FLAG_COMPRESS) && !(log->flags & LOG_FLAG_DELAYCOMPRESS)) {
