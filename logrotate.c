...
static int isSecureDir(const char *pathname, uid_t exempt_uid, gid_t exempt_gid, int depth)
{
    ...
    r = isSecureDirStep(pathname, exempt_uid, exempt_gid, depth);
...
{
    static char lastSecureDir[PATH_MAX];
    size_t pathlen;
    uid_t exempt_uid;
    gid_t exempt_gid;
    int r;
    ...
    if (strncmp(lastSecureDir, pathname, pathlen) == 0 &&
        (lastSecureDir[pathlen] == '\0' || lastSecureDir[pathlen] == '/'))
        return 1;
    ...
    r = isSecureDir(pathname, exempt_uid, exempt_gid, 0);
    if (r == 1 && pathlen < sizeof(lastSecureDir)) {
        strncpy(lastSecureDir, pathname, sizeof(lastSecureDir) - 1);
        lastSecureDir[sizeof(lastSecureDir) - 1] = '\0';
    }
    return r;
}
