...
duperror:
                    ...
                    if (newlog->oldDir) {
                        ...
                        for (j = 0; j < newlog->numFiles; j++) {
                            ...
                            /* Check if olddir and its parents of this log has safe permissions */
                            r = checkIsSecureDir(dirName, newlog);
                            if (r < 0 && (r != -ENOENT || (newlog->flags & LOG_FLAG_OLDDIRCREATE) == 0)) {
                                message(MESS_ERROR, "%s:%d failed checking whether olddir %s is a secure dir: %s\n",
                                        configFile, lineNum, dirName, strerror(-r));
                                free(ld);
                                goto error;
                            }
                            if (r == 0) {
