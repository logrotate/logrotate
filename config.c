...
static int readModeUidGid(const char *configFile, int lineNum, char *key,
	const char *directive, mode_t *mode, uid_t *uid, gid_t *gid) {
	char u[200], g[200];
	int m;
	char tmp;
	int rc;
			...
			if (checkFile(dp->d_name)) {
				...
				if (files_count % REALLOC_STEP == 0) {
					p = (char **) realloc(namelist,
						(files_count + REALLOC_STEP) * sizeof(char *));
					if (p) {
						namelist = p;
...
#ifdef HAVE_MADVISE
#ifdef MADV_DONTFORK
	madvise(buf, (size_t)(length + 2), MADV_SEQUENTIAL | MADV_WILLNEED | MADV_DONTFORK);
#else /* MADV_DONTFORK */
	madvise(buf, (size_t)(length + 2), MADV_SEQUENTIAL | MADV_WILLNEED);
#endif /* MADV_DONTFORK */
#endif /* HAVE_MADVISE */
			...
			case STATE_DEFAULT:
				...
				if (isalpha((unsigned char)*start)) {
					...
					if (!strcmp(key, "compress")) {
						...
						if ((key = isolateValue(configFile, lineNum, "extension name", &start,
							&buf, length)) != NULL) {
						...
						message(MESS_DEBUG, "compress_prog is now %s\n",
							...
							if (poptParseArgvString(options, &newlog->compress_options_count,
						...
						if (!(newlog->compress_ext =
							readPath(configFile, lineNum, "compress-ext", &start, &buf, length))) {
							RAISE_ERROR();
						}
					...
					for (argNum = 0; argNum < argc && logerror != 1; argNum++) {
						...
						newlog->files = realloc(newlog->files,
						...
						for (i = 0; i < globResult.gl_pathc; i++) {
							...
							for (log = logs.tqh_first; log != NULL; log = log->list.tqe_next) {
								for (k = 0; k < log->numFiles; k++) {
									if (!strcmp(log->files[k], globResult.gl_pathv[i])) {
										message(MESS_ERROR,
							...
							newlog->files[newlog->numFiles] = strdup(globResult.gl_pathv[i]);
							newlog->numFiles++;
						}
						duperror:
						globfree(&globResult);
